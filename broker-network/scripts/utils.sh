...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		...
		res=$?
                ...
                set -x
		peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -l ${LANGUAGE} -v 1.0 -c '{"Args":[]}' -P "AND	('Org1MSP.peer','Org2MSP.peer')" >&log.txt
		res=$?
                set +x
	fi
	cat log.txt
...
chaincodeQuery () {
  ...
  do
     ...
     set -x
     peer chaincode query -C $CHANNEL_NAME -n mycc -c '{"Args":["retrieve","key001"]}' >&log.txt
	 res=$?
     set +x
...
chaincodeInvoke () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
		peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME -n mycc -c '{"Args":["publish","key001","participant","this is description"]}' >&log.txt
		res=$?
                set +x
	else
                set -x
		peer chaincode invoke -o orderer.example.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -c '{"Args":["publish","key001","participant","this is description"]}' >&log.txt
		res=$?
                set +x
	fi
	cat log.txt
